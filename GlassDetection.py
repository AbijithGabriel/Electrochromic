...
# Function to save the cropped frame as an image
def save_cropped_image(frame_crop):
    ...
    filename = os.path.join(save_dir, f"glass_detection_{time.strftime('%Y%m%d_%H%M%S')}.jpg")
    # Save the cropped frame as an image
    cv2.imwrite(filename, frame_crop)
